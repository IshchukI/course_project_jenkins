...
//                sh 'docker rm $(docker ps -a -q)' //удвлить все контейнеры
                sh 'docker stop course_project_container'  //остановить все кнотейнеры
                sh 'docker rm course_project_container' //удвлить все контейнеры
        ...
        stage('Run project'){
            steps {
                ...
                sh 'docker exec course_project_container sh -c \"python3 -m pytest PlayWrite/test -p no:cacheprovider --alluredir=/tmp/reports/allure-results\"'
            }
        }
    }
}
